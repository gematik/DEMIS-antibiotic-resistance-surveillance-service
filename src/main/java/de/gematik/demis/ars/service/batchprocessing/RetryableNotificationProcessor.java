...
import static de.gematik.demis.ars.service.batchprocessing.config.RetryableConfig.KEY_DELAY;
import static de.gematik.demis.ars.service.batchprocessing.config.RetryableConfig.KEY_MAX_DELAY;
import static de.gematik.demis.ars.service.batchprocessing.config.RetryableConfig.KEY_MAX_RETRIES;
import static de.gematik.demis.ars.service.batchprocessing.config.RetryableConfig.KEY_MULTIPLIER;
...
class RetryableNotificationProcessor {
  ...
  static class ExcludeServiceCallClientErrors implements MethodRetryPredicate {
    ...
    public boolean shouldRetry(final @NonNull Method method, final @NonNull Throwable throwable) {
      if (throwable instanceof ServiceCallException serviceCallException) {
        return serviceCallException.getHttpStatus() >= 500;
      }
      return true;
    }
  }
