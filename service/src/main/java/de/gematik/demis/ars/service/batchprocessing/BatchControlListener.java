...
import de.gematik.demis.ars.service.batchprocessing.messages.BatchCloseMessage;
 ...
 * {@link BatchEntity} record for each received message. The listener is only activated when the
 * property {@code ars.batch-processing.enabled} is set to {@code true}.
 */
@Slf4j
@Component
...
public class BatchControlListener {
   ...
   * notifications from the JSON message body, creates a new {@link BatchEntity} entity and persists
   * it via {@link BatchRepository}.
   *
   * @param message JSON-encoded {@link BatchCloseMessage} payload containing {@code
   *     numberOfMessages}
  ...
  public void processMessage(String message, @Headers Map<String, Object> amqpHeaders) {
    ...
    try {
      ...
      log.error("{} - error saving control message. Reason: {}", batchId, ex.getMessage());
      throw new SaveResultFailedException(batchId + " - error saving close message.", ex);
    }
   ...
   *
   * @param message JSON-encoded {@link BatchCloseMessage}
   * @return the number of notifications declared in the message
   */
  private int extractNumberOfNotifications(String message) {
    BatchCloseMessage batchCloseMessage = objectMapper.readValue(message, BatchCloseMessage.class);
    return batchCloseMessage.numberOfMessages();
  }
}
